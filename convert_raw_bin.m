function convert_raw_bin(filenames,channels,doPos)
...
for file=1:length(filenames) 
    ...
    count=0;
    [chMat,posMat,recDurSamps,RBDLen] = binfile_refmatrix(channels,RBDLen,recDurSamps,doPos,f_from);
    for i=channels  %  (should add one for doPos)
        i_channels_fopen; % creates the fopen file handles; one file per individual channell
    end
    tic; 
